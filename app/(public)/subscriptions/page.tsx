...
const PLANS = [
    {
        ...
        price: 19,
        features: ['1 User', '5 Projects', '5GB Storage'],
        highlighted: false,
    ...
    },
    {
        ...
        price: 99,
        features: ['Unlimited Users', 'Unlimited Projects', '200GB Storage'],
        highlighted: false,
...
const Subscriptions = () => {
    ...
    return (
        <>
            ...
            <section className="relative py-14 bg-linear-to-b from-[#f3f6ff] to-[#eef2ff] overflow-hidden">
                {/* Glow */}
                ...
                <div className="relative max-w-7xl mx-auto px-6 text-center">
                    ...
                    <motion.div className="mt-16 grid gap-8 md:grid-cols-3" variants={containerVariants} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }}>
                        {PLANS.map((plan) => (
                            <motion.div key={plan.name} variants={itemVariants} whileHover={{ y: -8 }} className={`relative rounded-2xl bg-white px-8 py-10 text-left shadow-[0_20px_60px_-15px_rgba(0,0,0,0.15)] ${plan.highlighted ? 'scale-105 border-2 border-indigo-500' : ''}`}>
                                {plan.highlighted && (
                                ...
                                <div className="flex justify-center">
                                    <Image src={plan.image} alt={plan.name} width={160} height={160} loading="lazy" className="object-contain" />
                                </div>

                                <h3 className="mt-2 text-2xl text-center font-semibold text-slate-900">
                                    {plan.name}
                                </h3>
                                ...
                                <button
                                    className={`mt-8 w-full rounded-md py-2 font-medium cursor-pointer ${plan.buttonStyle}`}
                                >
                                    Get Started
                                </button>
                            </motion.div>
